...
def callPlot():
  global ycIpFid, ycIpNam, ycOpFid, yDatFid
  global spc2Fid, spc3Fid, vbl2Fid, vbl3Fid
  global allvLiftFid, allvDragFid, allvLvsDFid, allvDspdFid
  global Va, Aa, Ka, Ra, Fa                                    # Approach  parms
  global Vc, Hc, Kc, Rc                                        # Cruise    parms 
  global Cw, Iw, Aw, Ww, Pw, Lw, Dw, Lr, Dr                    # Wing/Ailr parms
  global Ch, Ih, Ah, Wh, Ph, Lh, Dh                            # Hstab     parms
...
#
  ...
  for versKywd in versDict.keys():
    ...
    vdatFid  = ycIpNam +  '-vdat-' + versSfix + '.txt'
    dspdFid  = ycIpNam +  '-dspd-' + versSfix + '.txt'
    ##
    ## # open auto yasim config file
    ...
    if ( versSfix == 'v2017-2') :
      styl = 'linespoints'
    line = line + '"' + dspdFid +'" every 4::3        using '            \
       + '1:2 with ' + styl + ' title \'DragVsSpd ' + versSfix + '\', \\\n'
    dspdHndl.write(line)
    versIter += 1
    ...
    #
    # run yasim external process for saved dataset file
    ddatHndl = open(dspdFid, 'w')
    command_line = 'yasim ' + vcfgFid + ' -d -a '+ str(Hy)
    args = shlex.split(command_line)
    p = subprocess.Popen(args, stdout=ddatHndl)
    ddatHndl.close
    p.wait()
    #
  ...
  DEVNULL.close()
  #
  # run gnuplot with all versions Drag vs Speed command file to plot dataset
  command_line = "gnuplot -p " + allvDspdFid
  args = shlex.split(command_line)
  DEVNULL = open(os.devnull, 'wb')
  p = subprocess.Popen(args, stdout=DEVNULL, stderr=DEVNULL)
  p.communicate()
